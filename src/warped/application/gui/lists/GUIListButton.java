...
public class GUIListButton extends WarpedGUI {
	...
	private Vec2i menuMaxSize = new Vec2i(120, 30);
	private Vec2i buttonSize = new Vec2i(120, 30);
	
	private ArrayList<WarpedOption> options; 
	...
	public void setButtonSize(Vec2i vec) {setButtonSize(vec.x,vec.y);}
	...
	public void setMenuSize(Vec2i vec) {setMenuSize(vec.x, vec.y);}
	public void setMenuSize(int x, int y) {
		menuMaxSize.set(x, y);
		buttonSize.set(x, buttonSize.y);
	}
	public void setOptions(ArrayList<WarpedOption> options) {
	...
	public void clear() {
		if(options == null) options = new ArrayList<WarpedOption>();
		else options.clear();
		updateGraphics();
		setScrollMax();
	}
	
	public void setOptions(ArrayList<WarpedOption> options, ArrayList<Boolean> optionLocks) {
	...
	public Vec2i getMenuSize() {return menuMaxSize;}
	...
	private void updateGraphics() {		
		...
		int totalButtonHeight = size() * (buttonSize.y + buttonPadding);
		if(totalButtonHeight <= 0) totalButtonHeight = 1;
		if(totalButtonHeight < menuMaxSize.y) img = new BufferedImage(menuMaxSize.x, totalButtonHeight, WarpedProperties.BUFFERED_IMAGE_TYPE);
		else  img = new BufferedImage(menuMaxSize.x, menuMaxSize.y, WarpedProperties.BUFFERED_IMAGE_TYPE);
		...
		for(int i = 0; i < size(); i++) {	// draw each button
			WarpedOption option = options.get(i);
